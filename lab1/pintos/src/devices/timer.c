 ...
 * Interrupts must be turned on. EDIT: must turn interrupts off
 */
void
timer_sleep (int64_t ticks) 
{
  // if ticks are less than 0, immediately return
  if(ticks <= 0) return;

  int64_t start = timer_ticks ();

  // save wakeup time (start + ticks) in current thread
  int64_t wakeup_time = start + ticks;
  struct thread *cur = thread_current();
  cur->wakeup = wakeup_time;
...
{
  ticks++;
  // During every tick, check to see if any threads in sleep list needs
  // to wake up. Unblock them if their wakeup time is past the
  // amount of ticks.
  thread_wake(ticks);
  thread_tick ();
}
